...
def find_missing_letter(chars):
    """Find missing letter by alphafit."""
    letter_order = frozenset(chr(idx) for idx in
                             range(ord(chars[0]), ord(chars[0]) + len(chars)))
    return [letter for letter in letter_order if letter not in chars][0]
